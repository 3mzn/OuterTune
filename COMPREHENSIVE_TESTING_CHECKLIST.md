...
- [x] Open "Send to Friends" dialog from song menu
- [x] Dialog displays list of friends
- [x] Can select/deselect multiple friends
- [x] "Send" button is disabled when no friends selected
- [x] "Send" button is enabled when friends selected
- [x] Song appears in recipient's "To Listen" playlist (verify in Firebase)
- [x] Sent song has correct metadata (title, artist, duration, thumbnail)
- [x] Can send same song to multiple friends at once
- [x] Can send different songs to same friend

### 1.2 UI/UX (Single Device)
- [x] Dialog closes after sending
- [x] Success message/toast appears
- [ ] Can send from song menu (3-dot menu)
- [x] Can send from selection mode (bulk send)
- [x] Dialog shows friend names correctly
- [x] Friend list updates if friends are added/removed
- [x] No crashes when sending

### 1.3 Error Handling (Single Device)
- [x] Graceful handling if no friends exist
- [x] Graceful handling if network is offline
- [x] Graceful handling if Firebase is unavailable
...
- [x] Received songs appear in "To Listen" playlist
- [x] Songs appear in correct order (by sentAt timestamp)
- [x] Duplicate songs can not be added (same song sent twice = two entries)
- [x] Songs from multiple friends appear together
- [x] Playlist updates in real-time when new songs arrive
- [x] No duplicate entries if same song received twice simultaneously
...
- [x] Room database reflects Firebase state
- [x] Songs with `completedAt != null` are filtered out
- [x] Songs with `listenedAt != null` but `completedAt == null` remain in playlist
- [x] Logout/login preserves "To Listen" playlist
- [x] App restart preserves "To Listen" playlist
...
- [x] Pausing and resuming doesn't reset progress
- [ ] Skipping to next song doesn't mark as completed
- [x] Skipping backwards doesn't reset progress
- [x] Seeking to 99% then to 50% doesn't trigger completion
- [x] Seeking to 100% manually triggers completion
